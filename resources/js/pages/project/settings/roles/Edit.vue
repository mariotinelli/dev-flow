...
import UpdateController from '@/actions/App/Http/Controllers/Project/Settings/Roles/UpdateController';
...
const props = defineProps<{
    projectRole: EditableProjectRole;
...
} = useRolePermissions(props.permissionGroups, props.projectRole.permissions);
...
<template>
    <div class="flex h-full flex-1 flex-col gap-6">
        <Head :title="`Editar ${projectRole.name}`" />

        <Heading title="Editar papel do projeto" description="Atualize o nome e as permissões deste papel." />

        <Form
            novalidate
            v-bind="UpdateController.form(projectRole.id)"
        ...
        >
            <div class="grid gap-2">
                ...
                <Input id="name" name="name" required :default-value="projectRole.name" />
            ...
            <div class="space-y-4">
                <div>
                    <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                        <div>
                            <h2 class="font-medium">Permissões do projeto</h2>
                            <p class="text-sm text-muted-foreground">
                                Marque as ações que este papel deve conceder dentro dos projetos.
                            </p>
                        </div>
