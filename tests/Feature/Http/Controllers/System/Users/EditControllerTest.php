...
test('guests are redirected from user editing', function () {
    ...
    $this->get(route('system.users.edit', $user))->assertRedirect(route('login'));
});
...
test('authenticated users without permission cannot view user editing', function () {
    ...
    $this->actingAs($user)->get(route('system.users.edit', $otherUser))->assertForbidden();
});
...
test('authenticated users can view user editing without admin role option', function () {
    ...
    $this->actingAs($user)
        ->get(route('system.users.edit', $otherUser))
        ->assertOk()
        ->assertInertia(
            fn (Assert $page) => $page
