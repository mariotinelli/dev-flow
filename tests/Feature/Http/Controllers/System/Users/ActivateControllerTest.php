...
test('guests are redirected when activating users', function () {
    ...
    $this->post(route('system.users.activate', $user))->assertRedirect(route('login'));
});
...
test('authenticated users without permission cannot activate users', function () {
    ...
    $this->actingAs($user)->post(route('system.users.activate', $otherUser))->assertForbidden();
...
test('authenticated users can activate users', function () {
    ...
    $this->actingAs($user)
        ->post(route('system.users.activate', $otherUser))
