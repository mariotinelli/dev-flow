...
test('guests are redirected from role edition', function () {
    ...
    $this->get(route('system.roles.edit', $role))->assertRedirect(route('login'));
});
...
test('users without permission cannot view role edition', function () {
    ...
    $this->actingAs($user)
        ->get(route('system.roles.edit', $role))
        ->assertForbidden();
});
...
test('admin role cannot be edited', function () {
    ...
    $this->actingAs($user)
        ->get(route('system.roles.edit', $adminRole))
        ->assertForbidden();
});
...
test('users with update role permission can view role edition', function () {
    ...
    $this->actingAs($user)
        ->get(route('system.roles.edit', $targetRole))
        ->assertOk();
});
