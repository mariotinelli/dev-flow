...
test('guests are redirected when deleting roles', function () {
    ...
    $this->delete(route('system.roles.destroy', $role))->assertRedirect(route('login'));
...
test('admin role cannot be deleted', function () {
    ...
    $this->actingAs($user)
        ->delete(route('system.roles.destroy', $adminRole))
        ->assertForbidden();
...
test('users without permission cannot delete roles', function () {
    ...
    $this->actingAs($user)
        ->delete(route('system.roles.destroy', $targetRole))
        ->assertForbidden();
