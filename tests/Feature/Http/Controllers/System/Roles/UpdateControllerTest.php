...
test('guests are redirected when updating roles', function () {
    ...
    $this->post(route('system.roles.update', $role), [
...
test('users without permission cannot update roles', function () {
    ...
    $this->actingAs($user)
        ->post(route('system.roles.update', $role), [
...
test('admin role cannot be updated', function () {
    ...
    $this->actingAs($user)
        ->post(route('system.roles.update', $adminRole), [
            'name'        => 'super-admin',
            'permissions' => [],
        ])
        ->assertForbidden();
...
test('users with update role permission can update roles', function () {
    ...
    $this->actingAs($user)
        ->post(route('system.roles.update', $targetRole), [
